package com.example.esercitazione21_02.controller;
...
import com.example.esercitazione21_02.repository.CourseRepository;
...
public class CourseController {
    ...
    public ResponseEntity<?> findOneCampo(@PathVariable long id, @PathVariable String campo) {
        ...
        return new ResponseEntity<>((campo.equalsIgnoreCase("descrizione")) ? c[0].getDescrizione() : c[0].getNome(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getNumeroCorsi_() {
        ...
        for (Course course : courseRepository.findAll())
            if (course.getNome().toLowerCase().contains("corso_")) i++;
